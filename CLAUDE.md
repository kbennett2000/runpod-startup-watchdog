...
- Failed startup is a real and recurring failure mode in public reports: pods that hang in STARTING or crash while the billing meter runs. The README links every report that was read. The set is small and skewed, so it demonstrates that the failure mode happens, and does not measure how often. The only account-level guardrail is a default $80/hour spend cap, seen in this project's own billing screen (demo/billing-before.png).
